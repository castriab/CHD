...
selected_columns <- places %>%
  select(CHD, BINGE, CSMOKING, LPA, SLEEP, CERVICAL, CHOLSCREEN, ACCESS2, COLON_SCREEN, MAMMOUSE, 
         COREM, COREW, BPMED, DENTAL, CHECKUP)

colnames(selected_columns) <- c('CHD', "Binge Drinking", "Smoking", "Physical Inactivity", "Sleep", 
                                "Cervical Screening", "Cholesterol Screening", "Health Insurance", 
                                "Colonoscopy", "Mammography", "Men Preventative", "Women Preventative", 
                                "BP Medication", "Dental Visit", "Checkup Visit")
